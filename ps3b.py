...
class Patient(object):
    ...
    def update(self):
        ...
        for virus in self.viruses:
            if virus.doesClear():
                clearedViruses.append(virus)
        for virus in clearedViruses:
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for numTrial in range(numTrials):
        ...
        for timestep in range(300):
            if numTrial == 0:
                valuesDict[timestep] = []
            valuesDict[timestep].append(P1.update())
    #plots results
    yValues = []
    for i in range(300):
        yValues.append(sum(list(valuesDict.values())[i])/numTrials)
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        NoChildException if this virus particle does not reproduce.
        """
        if all(self.resistances[drug] for drug in activeDrugs) and random.random() <= self.maxBirthProb * (1 - popDensity): #test resitances and reproduction prob
            resistances = {} #resistances inheritance
            ...
            return ResistantVirus(self.maxBirthProb, self.clearProb, 
                                  resistances, self.mutProb) #offspring
        else:
            raise NoChildException
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        resistantViruses = []
        for virus in self.viruses:
            if type(virus) == ResistantVirus: #do i need this condition?
